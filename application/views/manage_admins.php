...
<body data-sidebar="dark">
        ...
        <div class="main-content">
            ...
            <div class="page-content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Add Balance Modal -->
                    <div class="modal fade" id="add_balance_Modal" tabindex="-1" role="dialog"
                        aria-labelledby="add_balance_Modal" aria-hidden="true">
                        <div class="modal-dialog" role="document">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="add_balance_Modal">Add Balance</h5>
                                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                                    </button>
                                </div>
                                <div class="modal-body">
                                    <div class="panel-body">
                                        <input type="hidden" id="add_Balance_Id" name="add_balance_user_id" value="">
                                        <div class="form-group mb-3"> <label class="control-label">Add
                                                Balance</label>&nbsp;&nbsp;<input id="add_balance_input" type="number"
                                                name="label" class="form-control" placeholder="" required="">
                                        </div>
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                                    <button type="button" onclick="Update_Balance()" class="btn btn-primary">
                                        Save</button>
                                </div>
                            </div>
                        </div>
    ...
    <script>
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response != "") {
                        ...
                        $("#add_Balance_Id").val(response.id);
        ...
        function Update_Balance() {
            var add_Balance_Id = document.getElementById("add_Balance_Id").value; // User ID
            var add_balance_input = document.getElementById("add_balance_input").value; // Balance to add

            // Validate that the input fields are not empty
            if (!add_Balance_Id || !add_balance_input) {
                alert("Please enter a valid balance and user.");
                return;
            }

            // Preparing the data object to send via AJAX
            var jsonObjects = {
                add_Balance_Id: add_Balance_Id,
                add_balance_input: add_balance_input
            };

            $.ajax({
                type: 'POST',
                url: '<?= site_url('Admin_api/update_user_balance') ?>', // Updated URL
                data: jsonObjects,
                dataType: 'json',
                success: function(response) {
